...
def ISTA(A,b,x0,alpha,max_iter, step_size, tol, cost, prox, group_len, groups_dict_idx, approx_sol = 0):
  x = x0.copy()
  AT = A.T
  ...
  cost_val[0] = cost(A,x,b,alpha,groups_dict_idx)
  ...
  for i in range(max_iter):
    x = prox(x - step_size*grad, step_size*alpha,groups_dict_idx)

    time_list[i+1] = time.time() - start_time
    x_k[i+1] = np.linalg.norm(x-approx_sol)
    cost_val[i+1] = cost(A,x,b,alpha,groups_dict_idx)
    print('Iteration:', i, 'Cost:', cost_val[i+1])

    r = A @ x - b
    grad_new = AT @ r
    stopping_criteria = np.linalg.norm(x - prox(x - step_size*grad_new, step_size*alpha,groups_dict_idx))/ (1 + np.linalg.norm(x) + np.linalg.norm(r))
...
def BT_ISTA(A,b,x0,alpha,max_iter, tol, cost, prox, group_len,groups_dict_idx, approx_sol = 0):
  x = x0.copy()
  AT = A.T
...
def FISTA1(A, b, x0, alpha, max_iter, step_size, tol, cost, prox,group_len,groups_dict_idx, approx_sol = 0):
    x = x0.copy()
    x_old = x0.copy()
    z = x0.copy()
    AT = A.T
    ...
    for i in range(max_iter):
        rz = A @ z - b
        grad = AT @ rz
        x = prox(z - step_size*grad, alpha*step_size,groups_dict_idx)
        ...
        r = A @ x - b
        grad_xnew = AT @ r
        stopping_criteria = np.linalg.norm(x - prox(x - step_size*grad_xnew, step_size*alpha,groups_dict_idx))/ (1 + np.linalg.norm(x) + np.linalg.norm(r))
        if stopping_criteria < tol or abs(cost_val[i+1] - cost_val[i]) < tol:
...
def BT_FISTA1(A, b, x0, alpha, max_iter, tol, cost, prox, group_len, groups_dict_idx, approx_sol = 0):
    x = x0.copy()
    x_old = x0.copy()
    z = x0.copy()
    AT = A.T
    ...
    for i in range(max_iter):
        ...
        z = z - step_size*grad
        x = prox(z, alpha*step_size,groups_dict_idx)
        x_k[i+1] = np.linalg.norm(x-approx_sol)
        t_old = t
        t = (1 + np.sqrt(1 + 4*(t_old**2)))/2
        z = x + ((t_old - 1) / t) * (x - x_old)
...
def Algo_Newton_Ista(A,b,x0,alpha,max_iter, step_size, beta, newton_stepsize, tol, cost,
                     prox, subproblem_solver, group_len, groups_dict_idx, newt_tol = 1e-3, approx_sol = 0):
  x = x0.copy()
  AT = A.T
  ...
  time_list[0] = 0.0
  x_k[0] = np.linalg.norm(x-approx_sol)
  num_groups = len(groups_dict_idx)
  start_time = time.time()
  do_newton = 0
  ...
  for i in range(max_iter):
    x_hat = prox(x - step_size*grad, step_size*alpha,groups_dict_idx)

    if do_newton:
      Gradient_map = (x-x_hat)/step_size
      y = Gradient_map - grad
      d = subproblem_solver(A,x_hat,y,b,num_groups,group_len, alpha)
      newton_stepsize = 1
      x_new = x_hat - newton_stepsize*d
      if cost(A,x_new,b,alpha,groups_dict_idx) >= cost(A,x_hat,b,alpha,groups_dict_idx):
         x_new = x_hat
    else:
      x_new = x_hat
    ...
    cost_val[i+1] = cost(A,x_new,b,alpha,groups_dict_idx)
    print('Iteration:', i, 'Cost:', cost_val[i+1])
    ...
    r = A @ x_new - b
    grad_xnew = AT @ r
    stopping_criteria = np.linalg.norm(x_new - prox(x_new - step_size*grad_xnew, step_size*alpha,groups_dict_idx))/ (1 + np.linalg.norm(x_new) + np.linalg.norm(r))
    if stopping_criteria < tol or abs(cost_val[i+1] - cost_val[i]) < tol:
...
def Algo_Newton_BT_Ista(A,b,x0,alpha,max_iter, beta, newton_stepsize, tol, cost,
  ...
  for i in range(max_iter):
    step_size = backtracking_linesearch(A, b, x, grad, prox, alpha, groups_dict_idx)
    x_hat = prox(x - step_size*grad, step_size*alpha,groups_dict_idx)
...
def Algo_Newton_Fista_new(A,b,x0,alpha,max_iter, step_size, beta, newton_stepsize, tol, cost,
  ...
  x_k[0] = np.linalg.norm(x0-approx_sol)
  cost_val[0] = cost(A,x0,b,alpha,groups_dict_idx)
  time_list[0] = 0.0

  for i in range(max_iter):
    grad = AT @ (A @ z - b)
    x_hat = prox(z - step_size*grad, alpha*step_size,groups_dict_idx)
...
def Algo_Newton_BT_Fista_new(A,b,x0,alpha,max_iter, beta, newton_stepsize, tol, cost,
  ...
  for i in range(max_iter):
    grad = AT @ (A @ z - b)
    step_size = backtracking_linesearch(A, b, z, grad, prox, alpha, groups_dict_idx)
    x_hat = prox(z - step_size*grad, alpha*step_size,groups_dict_idx)

...
def classic_lasso_ssnal(Ainput, b, n, lam, options, y=None, xi=None, x=None):
    ...
    if Ascale != 0 and existA:
        if Ascale == 1:
            dscale = 1.0 / np.maximum(1, np.sqrt(np.asarray(A.multiply(A).sum(axis=0)).ravel()))
        elif Ascale == 2:
            dscale = 1.0 / np.sqrt(np.asarray(A.multiply(A).sum(axis=0)).ravel())
    ...
    parmain = {
        ...
        'orgojbconst': 0.0,             # if you have no “constant” term
        'approx_sol': approx_sol
    }
    #print('stop_TOLLL',stoptol)
    if existA:
        parmain['A'] = A
    if 'sigma' in options:
    ...
    obj_main, y, xi, x, info_main, runhist, x_history, time_history = Classic_Lasso_SSNAL_main(
    ...
    y = y * cscale
    x = x * bscale
    x_history = x_history * bscale
    if Ascaleyes:
        x = dscale * x
        x_history = x_history * dscale
        y = y / dscale
    ...
    if printyes:
        ...
        print("-" * 80)
    # print('x_hist_last', x_history[-1])
    # print('XXX', x)
    #print('primal_hist', runhist['primobj'])
    return [primobj, dualobj], y, xi, x, info, runhist, x_history, time_history

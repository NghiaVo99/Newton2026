import numpy as np
import time
from src.lasso.Classic_Lasso_SSNCG import *
from src.lasso.mexscale_wrapper import mexscale
from src.lasso.mexsigma_update_wrapper import mexsigma_update_Classic_Lasso_SSNAL
from src.lasso.Classic_Lasso_SSNCG import proj_inf
...
def Classic_Lasso_SSNAL_main(Amap0, ATmap0, b, lam, parmain, y, xi, x):
    ...
    for it in range(1, maxiter + 1):
        ...
        if max(primfeasorg, dualfeasorg) < 500.0 * max(1e-6, stoptol):
            ...
            if Ascaleyes:
                y, rr = proj_inf(dscale * x * bscale - grad, lambdaorg)
                etaorg = np.linalg.norm(grad + y)
                eta    = etaorg / (1.0 + np.linalg.norm(grad) + np.linalg.norm(dscale * x * bscale))
            else:
                y, rr = proj_inf(x * bscale - grad, lambdaorg)
                etaorg = np.linalg.norm(grad + y)
    ...
    # print('\n x_hist_last', x_history[-1])
    # print('X', x)
    # print('primal_hist', runhist['primobj'])
    return obj, y, xi, x, info, runhist, x_history, time_history

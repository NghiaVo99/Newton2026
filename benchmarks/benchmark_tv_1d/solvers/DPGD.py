    ...
    def run(self, callback):
        ...
        while callback():
            ...
            if isinstance(self.A, np.ndarray):
                ...
                v_tmp, _ = cg(
                    AtA, Aty + np.diff(v, append=0, prepend=0), x0=v_tmp, tol=tol_cg
                )
                v = np.clip(v + stepsize * np.diff(v_tmp), -self.reg, self.reg)

            if self.use_acceleration:
                v_acc[:] = v + (t_old - 1.0) / t_new * (v - v_old)

            if isinstance(self.A, np.ndarray):
                self.u = AtA_inv @ (Aty + np.diff(v, append=0, prepend=0))
            else:
                self.u, _ = cg(
                    AtA, Aty + np.diff(v, append=0, prepend=0), x0=self.u, tol=tol_cg
                )

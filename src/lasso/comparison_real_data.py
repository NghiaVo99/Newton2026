...
mat = loadmat(d4)   # mpg_scale_expanded7.mat
A = mat["A"]        # allow sparse
b = mat["b"]
L = np.linalg.norm(A, 2)**2
print('Lipschitz constant before: ', L)


# 1) center b, ℓ2-cap columns
b_mean = b.mean()
b_new = b - b_mean
#A, s = scale_columns_at_most_unit(A)
A, d = preprocess_A_scale1(A)


L = np.linalg.norm(A, 2)**2
print('Lipschitz constant after: ', L)
...
series = [
    ...
    # },
    # {
    #     "name": "GDFBE_LM", "color": "orange", "marker": "P",
...
gs  = fig.add_gridspec(2, 2, height_ratios=[20, 1], hspace=0.25, wspace=0.25)
...
axs[1].bar(methods_sorted, times_sorted, color=colors_sorted)
axs[1].set_yscale('log')
axs[1].set_ylabel("log(Runtime)")        
axs[1].set_xticks("")            

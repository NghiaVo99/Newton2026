import numpy as np
from src.lasso.SSNAL.SuiteLasso.ClassicLasso.Classic_Lasso_python.Classic_Lasso_lyn_solver import classic_lasso_linsys_solver
...
def proj_inf(x, lam):
    """
    Project x onto the ℓ∞ ball of radius lam. Returns:
      y  — the clipped values (each entry of x limited to [−lam, lam])
      rr — a boolean mask (True where the projection did not change x)
    """
    # Ensure lambda is nonnegative
    if lam < 0:
        raise ValueError("lambda must be nonnegative")

    # Convert x to a NumPy array (so that scalar, lists, or arrays all work)
    x_arr = np.asarray(x)

    if lam == 0:
        # If lam is zero, everything projects to 0
        y = np.zeros_like(x_arr)
    else:
        # Clip each entry of x to lie between −lam and +lam
        y = np.clip(x_arr, -lam, lam)

    # rr is True where the projected value equals the original
    rr = (y == x_arr)
